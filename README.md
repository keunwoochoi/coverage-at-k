...
1. Coverage-at-K (C@K) on raw counts, with a normalized area metric AUC-C@K.
2. Coverage-at-Q (C@Q) on probabilities, with a Difference-from-Uniform score (DfU).
...
## 4. Difference-from-Uniform (DfU)

We measure how far C@Q deviates from the uniform step. Let `q* = 1/C`. Define:

```
DfU(p) = C * [ ∫_{0}^{q*} (1 - C@Q(p)) dQ  +  ∫_{q*}^{1} C@Q(p) dQ ].
```

Because C@Q(p) and the uniform reference are proportions (already divided by C), DfU ∈ [0,1]:
...
* `deviation_from_uniform(probs)` → DfU
...
| Distribution (100 items, 4 cats) | C@0 | AUC-C@K | DfU  |
|----------------------------------|-----|---------|------|
...
Heuristics:
* AUC-C@K decreases smoothly with concentration; DfU increases.
...
* DfU depends only on normalized probabilities (ignores total mass and unseen possible categories).

## 8. Usage

```
python example.py      # Coverage-at-K & AUC-C@K
python example_caq.py  # Coverage-at-Q & DfU
```
...
* DfU is sensitive only to ordering and magnitudes of probabilities, not to sample size.
